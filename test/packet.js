...
const response = Buffer.from([
  0x29, 0x64, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x03,
  0x77, 0x77, 0x77, 0x01, 0x7a, 0x02, 0x63, 0x6e, 0x00, 0x00, 0x01, 0x00, 0x01,
  0xc0, 0x0c, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0x90, 0x00, 0x04, 0x36,
  0xde, 0x3c, 0xfc,
]);
...
test('Package#toIPv6', function () {
  assert.equal(
    Packet.toIPv6([10756, 20034, 512, 0, 0, 0, 0, 803]),
    '2a04:4e42:200::323',
  );
  assert.equal(
    Packet.toIPv6([10755, 45248, 3, 208, 0, 0, 5057, 61441]),
    '2a03:b0c0:3:d0::13c1:f001',
  );
  assert.equal(
    Packet.toIPv6([10752, 5200, 16387, 2055, 0, 0, 0, 8206]),
    '2a00:1450:4003:807::200e',
  );
  assert.equal(
    Packet.toIPv6([9734, 18176, 12552, 0, 0, 0, 44098, 10984]),
    '2606:4700:3108::ac42:2ae8',
  );
});
...
test('Package#toIPv6 RFC 5952 — leading-zero addresses', function () {
  ...
  assert.equal(
    Packet.toIPv6([0, 0, 0, 0, 0, 0xffff, 0xc000, 0x0201]),
    '::ffff:c000:201',
  );
});
...
test('Package#fromIPv6', function () {
  ...
  assert.deepEqual(Packet.fromIPv6('2a03:b0c0:3:d0::13c1:f001'), [
    '2a03',
    'b0c0',
    '0003',
    '00d0',
  ...
  assert.deepEqual(Packet.fromIPv6('2a00:1450:4003:807::200e'), [
    '2a00',
    '1450',
    '4003',
    '0807',
  ...
  assert.deepEqual(Packet.fromIPv6('2606:4700:3108::ac42:2ae8'), [
    '2606',
    '4700',
    '3108',
    '0',
  ...
  assert.deepEqual(Packet.fromIPv6('::'), [
    '0',
    '0',
    '0',
    '0',
  ...
  assert.deepEqual(Packet.fromIPv6('::2606:4700:3108'), [
    '0',
    '0',
    '0',
    '0',
  ...
  assert.deepEqual(Packet.fromIPv6('606:4700:3108::'), [
    '0606',
    '4700',
    '3108',
    '0',
...
test('EDNS.ECS#encode', function () {
  ...
  assert.deepEqual(
    b,
    Buffer.from([
      0x00, 0x00, 0x29, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0b, 0x00,
      0x08, 0x00, 0x07, 0x00, 0x01, 0x18, 0x00, 0x0a, 0x0b, 0x0c,
    ]),
  );
});
...
test('EDNS.ECS#decode family=2 (IPv6)', function () {
  ...
  const buffer = Buffer.from([0x00, 0x02, 0x20, 0x00, 0x20, 0x01, 0x0d, 0xb8]);
  const reader = new Packet.Reader(buffer);
...
test('Packet.createResponseFromRequest does not mutate request', function () {
  ...
  assert.notStrictEqual(
    response,
    request,
    'response should be a distinct object',
  );
  assert.equal(
    request.header.qr,
    originalQr,
    'request.header.qr should not be mutated',
  );
  assert.equal(
    request.additionals.length,
    originalAdditionalsLength,
...
test('Packet.uuid exercises the full 16-bit range with high diversity', function () {
  ...
  assert.ok(
    samples.size > 4000,
    `expected high diversity, got ${samples.size}`,
  );
  for (let q = 0; q < 4; q++) {
    assert.ok(
      quartile[q] > 200,
      `quartile ${q} underrepresented (${quartile[q]}/5000)`,
    );
  }
});
...
test('EDNS exposes extendedRcode / version / doFlag', function () {
  const opt = new Packet.Resource.EDNS([], {
    extendedRcode: 16,
    version: 0,
    doFlag: true,
  });
...
test('EDNS.ECS#encode truncates IPv4 address to prefix length (RFC 7871)', function () {
  ...
  for (const [cidr, expectedOctets] of [
    ...
    const query = new Packet.Resource.EDNS([
      new Packet.Resource.EDNS.ECS(cidr),
    ]);
...
test('Packet#encode compresses repeated names (RFC 1035 §4.1.4)', function () {
  ...
  assert.equal(
    buf[ansNameStart],
    0xc0,
    'answer name should start with pointer high byte',
  );
...
test('Packet#encode compresses common suffixes (RFC 1035 §4.1.4)', function () {
  ...
  assert.equal(
    buf[ansNameStart + 2] & 0xc0,
    0xc0,
    'third byte starts a compression pointer',
  );
});
...
test('Packet#encode compresses names inside rdata (CNAME)', function () {
  ...
  assert.equal(
    rdlength,
    2,
    'CNAME pointing to existing name should compress to 2 bytes',
  );
});
...
test('Resource#decode clamps TTL with the sign bit set (RFC 2181 §8)', function () {
  ...
  assert.equal(
    ...
    0x7fffffff,
    'high-bit TTL must be clamped to 2^31 - 1',
  );
});
